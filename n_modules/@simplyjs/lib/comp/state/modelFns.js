...
var warnNotArray = (model, arr, fn, name) => model.prov.warn('model fn (' + fn + '): ' + name + ' of type (' + arr?.constructor?.name + '), expected (Array)');

var warnIndGT = (model, fn, ind, length) => model.prov.warn('model fn (' + fn + '): index (' + ind + ') greater than length ( ' + length + ' )');
...
var modelFns = {
	...
	toggle (model, value, checklist) {//toggle between checklist
		if (!Array.isArray(checklist)) {
			...
			if (checklist.length === 0) model.prov.warn('model fn (toggle): checklist is empty');
